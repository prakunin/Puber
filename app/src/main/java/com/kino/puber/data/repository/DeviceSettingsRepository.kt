...
internal class DeviceSettingsRepository(
    ...
    override suspend fun updateDeviceSetting(deviceId: Long, type: DeviceSettingType, optionId: Int) {
        when (type) {
            DeviceSettingType.STREAMING_TYPE ->
                kinoPubApiClient.updateDeviceSettings(deviceId, streamingType = optionId)
            DeviceSettingType.SERVER_LOCATION ->
                kinoPubApiClient.updateDeviceSettings(deviceId, serverLocation = optionId)
            ...
            DeviceSettingType.MIXED_PLAYLIST ->
                kinoPubApiClient.updateDeviceSettings(deviceId, mixedPlaylist = optionId)
        }.getOrThrow()
    }
}