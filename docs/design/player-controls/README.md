...
- Three shapes for the right-hand group, since audio, video and info already open the same
  panel — see `PlayerSettingsPanels.kt`, which says so in as many words.
...
- **Regrouping the row by meaning** — content on the left (episodes, about, watched), transport
  in the middle, a single gear on the right. Agreed on the stand, not ported: it collapses
  `ActivePanel.AudioSubtitles`/`VideoSettings` into one, repoints `Info` at the item's own
  description, and touches 11 files in `main` plus 6 tests.
