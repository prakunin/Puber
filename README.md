...
Проект начинался как форк [Puber](https://github.com/rovkinmax/Puber), но сейчас развивается самостоятельно и заметно
отличается от исходной кодовой базы. Основные направления этой версии:

- более предсказуемая TV-навигация, управление фокусом и восстановление состояния экранов;
- быстрая загрузка интерфейса, персистентный кэш и локальная синхронизация статуса просмотра;
- развитие плеера, настроек устройства и поведения приложения на реальном Android TV;
- собственные процессы сборки, тестирования и выпуска.

Оригинальный проект и этот форк развиваются независимо, поэтому их возможности, настройки, версии и готовые сборки
могут заметно различаться. Вопросы и предложения, относящиеся к этой версии приложения, следует оставлять в
[issues этого репозитория](https://github.com/prakunin/Puber/issues).
...
Локальные секреты можно передать через `.env`, `local.properties` или переменные окружения — именно в этом порядке приоритета:
...
[issue](https://github.com/prakunin/Puber/issues) в этом репозитории.
